-- =============================================================================
-- The Vault — Full Schema (run once in Supabase SQL Editor)
-- Combines all migrations + storage policies in order.
-- =============================================================================
...
CREATE TYPE public.commission_status AS ENUM (
  'inquiry', 'intake', 'design_review', 'concept_creation', 'cad_development',
  'render_approval', 'production', 'quality_control', 'shipped', 'completed', 'cancelled'
);

CREATE TYPE public.piece_type AS ENUM ('pendant', 'ring', 'bracelet', 'necklace', 'custom');

CREATE TYPE public.commission_story_type AS ENUM (
  'transformation', 'achievement', 'legacy', 'relationship', 'milestone', 'symbolic', 'other'
);

CREATE TYPE public.commission_file_category AS ENUM (
  'reference', 'inspiration', 'sketch', 'cad', 'render', 'approval',
  'production', 'qc', 'shipping', 'invoice', 'other'
);

CREATE TYPE public.message_sender_type AS ENUM ('client', 'staff', 'system');

CREATE TYPE public.payment_type AS ENUM (
  'design_deposit', 'milestone', 'production_final', 'adjustment', 'refund'
);
...
CREATE TYPE public.production_update_type AS ENUM (
  'milestone', 'note', 'delay', 'qc_pass', 'qc_fail', 'shipment'
);
...
CREATE POLICY "Clients can view own commissions" ON public.commissions FOR SELECT USING (auth.uid() = client_id);
CREATE POLICY "Staff can view all commissions" ON public.commissions FOR SELECT USING (public.is_staff());
CREATE POLICY "Clients can create commissions" ON public.commissions FOR INSERT WITH CHECK (auth.uid() = client_id);
CREATE POLICY "Staff can create commissions" ON public.commissions FOR INSERT WITH CHECK (public.is_staff());
CREATE POLICY "Clients can update own commissions in early stages" ON public.commissions FOR UPDATE
  USING (auth.uid() = client_id AND status IN ('inquiry', 'intake'));
CREATE POLICY "Staff can update all commissions" ON public.commissions FOR UPDATE USING (public.is_staff());
CREATE POLICY "Admins can delete commissions" ON public.commissions FOR DELETE USING (public.is_admin());

-- ========== 004: Files, messages, status history ==========
...
CREATE INDEX commission_status_history_commission_id_idx ON public.commission_status_history (commission_id);
...
CREATE POLICY "Clients view visible files" ON public.commission_files FOR SELECT
  USING (is_client_visible = true AND EXISTS (
    SELECT 1 FROM public.commissions c WHERE c.id = commission_id AND c.client_id = auth.uid()
  ));
CREATE POLICY "Staff view all files" ON public.commission_files FOR SELECT USING (public.is_staff());
CREATE POLICY "Clients upload files" ON public.commission_files FOR INSERT
  WITH CHECK (EXISTS (
    SELECT 1 FROM public.commissions c WHERE c.id = commission_id AND c.client_id = auth.uid()
  ));
CREATE POLICY "Staff manage files" ON public.commission_files FOR ALL USING (public.is_staff());

CREATE POLICY "Clients view messages" ON public.commission_messages FOR SELECT
  USING (is_internal = false AND EXISTS (
    SELECT 1 FROM public.commissions c WHERE c.id = commission_id AND c.client_id = auth.uid()
  ));
CREATE POLICY "Staff view all messages" ON public.commission_messages FOR SELECT USING (public.is_staff());
CREATE POLICY "Clients send messages" ON public.commission_messages FOR INSERT
  WITH CHECK (sender_id = auth.uid() AND is_internal = false AND EXISTS (
    SELECT 1 FROM public.commissions c WHERE c.id = commission_id AND c.client_id = auth.uid()
  ));
CREATE POLICY "Staff manage messages" ON public.commission_messages FOR ALL USING (public.is_staff());

CREATE POLICY "Clients view status history" ON public.commission_status_history FOR SELECT
  USING (EXISTS (
    SELECT 1 FROM public.commissions c WHERE c.id = commission_id AND c.client_id = auth.uid()
  ));
CREATE POLICY "Staff view status history" ON public.commission_status_history FOR SELECT USING (public.is_staff());
CREATE POLICY "Staff insert status history" ON public.commission_status_history FOR INSERT WITH CHECK (public.is_staff());

-- ========== 005: Payments, design, production ==========
...
CREATE POLICY "Clients view own payments" ON public.commission_payments FOR SELECT USING (auth.uid() = client_id);
CREATE POLICY "Staff view all payments" ON public.commission_payments FOR SELECT USING (public.is_staff());
CREATE POLICY "Staff manage payments" ON public.commission_payments FOR ALL USING (public.is_staff());

CREATE POLICY "Clients view concepts" ON public.design_concepts FOR SELECT
  USING (EXISTS (SELECT 1 FROM public.commissions c WHERE c.id = commission_id AND c.client_id = auth.uid()));
CREATE POLICY "Staff manage concepts" ON public.design_concepts FOR ALL USING (public.is_staff());

CREATE POLICY "Clients view CAD" ON public.cad_revisions FOR SELECT
  USING (EXISTS (SELECT 1 FROM public.commissions c WHERE c.id = commission_id AND c.client_id = auth.uid()));
CREATE POLICY "Staff manage CAD" ON public.cad_revisions FOR ALL USING (public.is_staff());

CREATE POLICY "Clients view production updates" ON public.production_updates FOR SELECT
  USING (is_client_visible = true AND EXISTS (
    SELECT 1 FROM public.commissions c WHERE c.id = commission_id AND c.client_id = auth.uid()
  ));
CREATE POLICY "Staff manage production updates" ON public.production_updates FOR ALL USING (public.is_staff());

-- ========== Storage ==========
INSERT INTO storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
VALUES (
  'commission-files', 'commission-files', false, 52428800,
  ARRAY['image/jpeg', 'image/png', 'image/webp', 'image/heic', 'application/pdf', 'model/stl', 'model/obj', 'application/octet-stream']
)
ON CONFLICT (id) DO NOTHING;

CREATE POLICY "Clients upload to own commission folder" ON storage.objects FOR INSERT
  WITH CHECK (
    bucket_id = 'commission-files' AND auth.role() = 'authenticated'
    AND EXISTS (SELECT 1 FROM public.commissions c WHERE c.id::text = (storage.foldername(name))[1] AND c.client_id = auth.uid())
  );

CREATE POLICY "Clients read own commission files" ON storage.objects FOR SELECT
  USING (
    bucket_id = 'commission-files' AND auth.role() = 'authenticated'
    AND EXISTS (SELECT 1 FROM public.commissions c WHERE c.id::text = (storage.foldername(name))[1] AND c.client_id = auth.uid())
  );

CREATE POLICY "Staff full access to commission files" ON storage.objects FOR ALL
  USING (bucket_id = 'commission-files' AND public.is_staff());
