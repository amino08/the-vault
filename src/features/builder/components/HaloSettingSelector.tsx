...
import { HALO_STYLE_PRESETS } from "@/features/builder/style-presets";
import type { HaloStyleId } from "@/features/builder/types";

interface HaloSettingSelectorProps {
  value: HaloStyleId;
  disabled?: boolean;
  onChange: (haloStyle: HaloStyleId) => void;
}

export function HaloSettingSelector({ value, disabled, onChange }: HaloSettingSelectorProps) {
  return (
    <section className="space-y-5">
      <div>
        <p className="text-[10px] uppercase tracking-[0.28em] text-vault-gold">Center Setting</p>
        <p className="mt-1 font-serif text-lg text-vault-ivory">Halo Setting</p>
      </div>
      ...
      {disabled && (
        <p className="text-xs tracking-wide text-vault-pearl/45">
          Enable a center stone to add a halo setting.
        </p>
      )}
    </section>
  );
